	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float2 targetPoint;
				float minlen = 10;
				float amp = 0.2 * sin(_Time.y*_Value) + 0.5;
                for(float i=0; i<2.0; i+= 0.1)
                {
                	
                    float x = amp * cos(_Time.y*_Value + i*PI);
                    float y = amp * sin(_Time.y*_Value + i*PI);
                ...
                pixel = fixed3((targetPoint.x+1.0)/2.0, (targetPoint.y+1.0)/2.0, 0.1+0.4*sin(_Time.y*_Value*1.5));
